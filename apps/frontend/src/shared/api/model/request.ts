  ...
  (response: AxiosResponse<BaseResponse<any>>) => {
    ...
    if (response.data?.user?.allowedPhone === false) {
      const errorData: ErrorEventEmitter = {
        action: 'bottom-sheet',
        description:
          'Для продолжения работы необходимо разрешить доступ к телефону в боте приложения. Без данного разрешения вы не увидите приглашения в организации. Для разрешения доступа к телефону перейдите в бот, нажмите на кнопку "меню" и выберите "Авторизация по номеру телефона"',
